...
class AddressNormalizer:
    ...
    def parse_house_number(self, text: str) -> Dict[str, Optional[str]]:
        ...
        # Поддержка формата дробных номеров вида "30/2":
        # интерпретируем как "30 к2" (по сути, отдельный корпус).
        # Важно делать это ПОСЛЕ приведения обозначений, чтобы не задеть уже нормализованные формы.
        t = re.sub(r'(\d+[а-яa-z]?)\s*/\s*(\d+[а-яa-z]?)', r'\1 к\2', t)
